...
#ifndef PROTOCOLS_DEBUGRINGSHARE_H_
#define PROTOCOLS_DEBUGRINGSHARE_H_
...
template<int K>
class DebugRingShare : public Rep3Share<Z2<K>>
{
    typedef DebugRingShare This;
    typedef Z2<K> T;
...
public:
    ...
    typedef GC::DebugSecret bit_type;

    // opening facility
    
    typedef DebugProtocol<DebugRingShare> Protocol;

    // preprocessing facility
    typedef ReplicatedPrep2k<DebugRingShare> LivePrep;

    // private input facility
    typedef ReplicatedInput<DebugRingShare> Input;

    // default private output facility (using input tuples)
    typedef SpecificPrivateOutput<DebugRingShare> PrivateOutput;

    typedef ReplicatedPO<This> PO;
    typedef DebugRingShare Honest;
    ...
    {
        return "Debug " + T::type_string();
    }

    // used for preprocessing storage location
    static string type_short()
    {
        return "DD" + string(1, clear::type_char());
    }
    ...
    template<class U>
    DebugRingShare(const FixedVec<U, 2>& other)
    {
    ...
    static void split(vector<U>& dest, const vector<int>& regs, int n_bits,
            const DebugRingShare* source, int n_inputs,
