...
#ifndef PROTOCOLS_TESTRINGSHARE_H_
#define PROTOCOLS_TESTRINGSHARE_H_
...
template<int K>
class TestRingShare : public TestShare<Z2<K>>
{
    typedef TestRingShare This;
    typedef Z2<K> T;
...
public:
    ...
    // opening facility
    typedef ReplicatedMC<TestRingShare> MAC_Check;
    typedef MAC_Check Direct_MC;

    // multiplication protocol
    typedef TestProtocol<TestRingShare> Protocol;

    // preprocessing facility
    typedef ReplicatedPrep2k<TestRingShare> LivePrep;

    // private input facility
    typedef ReplicatedInput<TestRingShare> Input;

    // default private output facility (using input tuples)
    typedef SpecificPrivateOutput<TestRingShare> PrivateOutput;

    typedef ReplicatedPO<This> PO;
    typedef TestRingShare Honest;
    ...
    TestRingShare() {
        if (BUILDING_SHARE_PROCESS & SEMI_RING_SHARE_PROCESS) {
            cout << "In TestRingShare()" << endl;
        }
    }

    template<class U>
    TestRingShare(const FixedVec<U, 2>& other)
    {
        if (BUILDING_SHARE_PROCESS & SEMI_RING_SHARE_PROCESS) {
            cout << "In TestRingShare(FixedVec<U, 2>)" << endl;
    ...
    static void split(vector<U>& dest, const vector<int>& regs, int n_bits,
            const TestRingShare* source, int n_inputs,
            typename U::Protocol& protocol)
    {

        // cout << "In TestRingShare::split()" << endl;
