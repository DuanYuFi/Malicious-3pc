...
template<class T>
void SemiRingProtocol<T>::prepare_mul(const T& x,
        const T& y, int n)
{
...
template<class T>
void SemiRingProtocol<T>::prepare_reshare(const typename T::clear& share,
        int n)
{
...
template<class T>
void SemiRingProtocol<T>::start_exchange()
{
    P.send_relative(1, os[0]);
    this->rounds++;
}
...
template<class T>
void SemiRingProtocol<T>::stop_exchange()
{
    P.receive_relative(-1, os[1]);
}
