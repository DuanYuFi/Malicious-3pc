...
DZKProof prove(
    ...
    for(uint64_t i = 0; i < k; i++) {
        for(uint64_t j = 0; j < s; j++) {
            input_left[i][4 * j] = Mersenne::mul(input_left[i][2 * j], eta_power);
            input_left[i][4 * j + 1] = Mersenne::mul(input_left[i][2 * j + 1], eta_power);
            input_left[i][4 * j + 2] = Mersenne::mul(input_left[i][2 * j + 1], eta_power);
            input_left[i][4 * j + 3] = Mersenne::mul(input_left[i][2 * j + 1], eta_power);
...
int main() {
    ...
    for (int i = 0; i < k; i++) {
        ...
        for (int j = 0; j < cols; j++) {
            if (temp_pointer >= T) {
                input_left[i][j * 4] = input_left[i][j * 4 + 1] = input_left[i][j * 4 + 2] = input_left[i][j * 4 + 3] = 0;
                input_right[i][j * 4] = input_right[i][j * 4 + 1] = input_right[i][j * 4 + 2] = input_right[i][j * 4 + 3] = 0;
                input_left_next[i][j * 4] = input_left_next[i][j * 4 + 1] = input_left_next[i][j * 4 + 2] = input_left_next[i][j * 4 + 3] = 0;
                input_right_prev[i][j * 4] = input_right_prev[i][j * 4 + 1] = input_right_prev[i][j * 4 + 2] = input_right_prev[i][j * 4 + 3] = 0;
                input_mono_prev[i][j * 4] = input_mono_prev[i][j * 4 + 1] = input_mono_prev[i][j * 4 + 2] = input_mono_prev[i][j * 4 + 3] = 0;
                input_mono_next[i][j * 4] = input_mono_next[i][j * 4 + 1] = input_mono_next[i][j * 4 + 2] = input_mono_next[i][j * 4 + 3] = 0;
                ...
                // Share with P_{i+1}
                input_left[i][j * 4] = x_first;
                input_left[i][j * 4 + 1] = y_first;
                input_left[i][j * 4 + 2] = y_first;
                input_left[i][j * 4 + 3] = y_first;
                // Share with P_{i-1}
                input_right[i][j * 4] = y_second;
                input_right[i][j * 4 + 1] = x_second;
                input_right[i][j * 4 + 2] = y_first;
                input_right[i][j * 4 + 3] = y_first;

                input_left_next[i][j * 4] = x_first;
                input_left_next[i][j * 4 + 1] = y_first;
                input_left_next[i][j * 4 + 2] = y_first;
                input_left_next[i][j * 4 + 3] = y_first;

                input_right_prev[i][j * 4] = y_second;
                input_right_prev[i][j * 4 + 1] = x_second;
                input_right_prev[i][j * 4 + 2] = x_second;
                input_right_prev[i][j * 4 + 3] = x_second;
    ...
    cout << "commu: " << (proof.get_size() + other_vermsg.get_size()) * sizeof(uint64_t)  << endl;
